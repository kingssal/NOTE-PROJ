...
public:
    ...
    void printLine(const std::string& input) {// input은 wordDB에서 얻어온 문자열 apple | 1:2, 1:3 이런 형식 사용
       // 단어에 대해서 가중치 표현하는 함수
    ...
    void addBranch(const std::string& fileName) { // 파일을 디렉토리에 복사하여 저장함. 파일 값이 변할 때 마다 호출해야함
        try {
    ...
    void deleteBranch(const std::string& fileName) {//파일을 디렉토리에서 삭제함.
        try {
    ...
    void show() {// 디렉토리 안에 있는 파일 이름을 출력함 // 
        try {
...
private:
    void createDirectory() { // RoadMap을 선언하면 디렉토리를 생성함
        try {
